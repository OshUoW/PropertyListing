...
export default function Properties() {
  ...
  // Function to determine if the property's price is within the given range
  const isWithinPriceRange = (price, range) => {
    const [min, max] = range.split('-').map(price => parseInt(price.replace(/[$,]/g, '')));
    return price >= min && price <= max;
  };

  // Function to get the index of a month by name for date comparisons
  ...
  // Renders the properties search panel, favorite toggle, and filtered properties list
  return (
    <div>
      <Navbar />
      <div className='properties'>
        <div className="search">
          <div className="search-fields">
            ...
            <input type="date" name="toDate" placeholder="To Date" onChange={handleInputChange} />c
        ...
        <div
          ...
          onDragOver={handleDragOver}
        >
          <div className="box-container" style={{ gridTemplateColumns: '1fr 1fr' }}>
...
{/* Favorites List Container: Shows or hides the favorites list based on user interaction */}
<div className={`favorites-list ${showFavorites ? 'show' : ''}`} onDrop={handleDrop} onDragOver={handleDragOver}>
<h3 className="favorites-header">Favourite Section</h3> {/* Add this line */}
  {favorites.map((property) => (
    <div
      className="propboxes favorite-card"
      key={property.id}
      draggable
      onDragStart={(event) => handleDragStart(event, property)} // Enables dragging functionality for each favorite property
      onDragEnd={handleDragEnd}
    >
      <div className="thumb" style={{ backgroundImage: `url(${property.picture})` }}></div>
      <div className="property-details">
        ...
        <div className="flex">
          <p><i className="fas fa-bed"></i>{property.bedrooms} Beds</p>
          <p><i className="fas fa-bath"></i>2 Baths</p>
        </div>
        <p className="price">${property.price}</p>
      </div>
      <button
        className="remove-btn"
        onClick={() => handleFavoriteToggle(property)} // Button to remove the property from favorites
      >
        Remove
      </button>
    </div>
